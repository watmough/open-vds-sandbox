...
TEST(IOTests, CreateSyntheticVDSWithoutMetadataPagesAndVerifyUpload)
{
  GTEST_SKIP() << "This test has to be enabled manually";
  ...
  OpenVDS::FloatVector3 frequency = OpenVDS::FloatVector3(0.6f, 2.f, 4.f);
  ...
  OpenVDS::GetAccessManager(inMemoryVDS).Flush(error);
  ASSERT_EQ(error.code, 0) << error.string;
  ...
  {
    OpenVDS::VolumeDataAccessManager networkAccessManager = OpenVDS::GetAccessManager(networkVDS);
    std::shared_ptr<OpenVDS::VolumeDataPageAccessor> networkPageAccessor = networkAccessManager.CreateVolumeDataPageAccessor(OpenVDS::Dimensions_012, 0, 0, 100, OpenVDS::VolumeDataAccessManager::AccessMode_Create, 0);
