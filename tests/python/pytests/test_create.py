...
def test_create_1bit_vds():
    ...
    #        print("min {} max {}".format(min, max))
            buf[:,:,:] = np.packbits(data[min[2]:max[2],min[1]:max[1],min[0]:max[0]], axis=-1, bitorder='little')
            page.release()
        accessor.commit()
    ...
    req = manager.requestVolumeSubset(min=(0,0,0), max=(shape[0], shape[1], shape[2]), format=openvds.VolumeDataFormat.Format_1Bit)
    result = np.unpackbits(req.data, count=np.prod(shape), bitorder='little').reshape(shape)
