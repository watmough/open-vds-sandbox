...
def test_2d_poststack_volume_info(poststack_2d_segy, output_vds):
    ...
    with openvds.open(output_vds.filename, "") as handle:
        ...
        assert descriptor.coordinateMin == 62012.0
        assert descriptor.coordinateMax == 63988.0
        ...
        # when primary axis is ensemble numbers this metadata is omitted
        assert not layout.isMetadataBLOBAvailable(openvds.KnownMetadata.ensembleNumbers().category,
                                              openvds.KnownMetadata.ensembleNumbers().name)


def test_2d_poststack_unitless_axis(poststack_2d_segy, output_vds):
    ex = ImportExecutor()
    ...
    with openvds.open(output_vds.filename, "") as handle:
        ...
        # when primary axis is 1..N  this metadata should be present
...
def test_2d_prestack_unitless_axis(prestack_2d_segy, output_vds):
    ex = ImportExecutor()
...
def test_crs_wkt(poststack_2d_segy, output_vds, crs_wkt):
    ex = ImportExecutor()
