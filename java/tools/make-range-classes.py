...
def createCopyCtor(typename: str, count: int) -> str:
    ...
    contents = """
    public TYPENAMERange(TYPENAMERange rhs) {
        this.createByteBuffer(JAVATYPE.BYTES * VECTORCOUNT);
        this.set(INIT);
    }
""".replace("INIT", init)
...
def createEquals(typename: str, count: int) -> str:
    ...
    public boolean equals(Object other) {
        ...
        TYPENAMERange real_other = (TYPENAMERange)other;
...
public class TYPENAMERange extends ByteBufferBackedObject {
"""
        for handler in HANDLERS:
            content += handler(t, n)
        content += "}\n"
        content = transformTemplate(content, t, n)
        class_filename = transformTemplate("TYPENAMERange.java", t, n)
