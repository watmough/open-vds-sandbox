...
#ifndef CPPJNI_COMMON_H_INCLUDED
#define CPPJNI_COMMON_H_INCLUDED

#ifndef CPPJNI_H_INCLUDED
#error "Never include this header by itself. Use CPPJNI.h"
#endif

#ifndef JAVA_WRAPPER_GENERATOR
#error "JAVA_WRAPPER_GENERATOR must be defined at the top of CPPJNI.h"
#endif
...
void CPPJNI_HandleSharedLibraryException(JNIEnv* env, CPPJNILibraryException& e);
...
#define CPPJNI_CATCH \
 ...
 catch(CPPJNILibraryException& e) { CPPJNI_HandleSharedLibraryException(env, e); } \
...
jobject       CPPJNI_createJavaObject(jclass clazz);
jobjectArray  CPPJNI_createJavaArray(int elements, const char* elementType = nullptr, jobject initialElement = 0);

template<typename T> 
jobject CPPJNI_createPODJavaObject(T const& value);

template<> jobject CPPJNI_createPODJavaObject<int>(int const& value);
template<> jobject CPPJNI_createPODJavaObject<int64_t>(int64_t const& value);

enum class CPPJNIExceptionType
{
  Exception,
  RuntimeException,
  IOException,
...
void CPPJNI_Throw(struct JNIEnv_ *env, const char* message, CPPJNIExceptionType exceptionType);
