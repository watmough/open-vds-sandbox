...
Json::Value SerializeMetadataStatus(MetadataStatus const &metadataStatus)
{
  ...
  {
    metadataStatusJson["validChunkCount"]      = metadataStatus.m_validChunkCount;
  }
...
bool ParseLayerStatus(const std::vector<uint8_t> &json, VDS &vds, LayerMetadataContainer &layerMetadataContainer, Error &error)
{
  ...
  try
  {
    for (const Json::Value &layerStatus : root)
    {
      ...
      metadataStatus.m_hasValidChunkCount = layerStatus.isMember("validChunkCount");
      metadataStatus.m_validChunkCount = layerStatus["validChunkCount"].asInt();
      ...
      metadataStatus.m_hasSerializedSize = layerStatus.isMember("serializedSize");
