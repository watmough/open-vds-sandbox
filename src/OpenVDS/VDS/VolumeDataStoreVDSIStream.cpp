...
#include "VolumeDataStoreVDSIStream.h"
...
VolumeDataStoreVDSIStream::VolumeDataStoreVDSIStream(VDS &vds, const void *vdsIStream, Mode mode, Error &error)
     : VolumeDataStoreVDSFile(vds, std::string(), mode, error)  // will error out with filename of ""
  ...
  // , m_dataStore(nullptr, &HueBulkDataStore::Close)
{
  ...
  {
    m_dataStore.reset(HueBulkDataStore::Open(vdsIStream));
    if (mode == ReadWrite)
    {
      assert(false && "Unimplemented");
    }
